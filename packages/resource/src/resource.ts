import {CheckPhase, checkPhase, Inject} from "@mmuscat/angular-composition-api";
import {catchError} from "rxjs/operators";
import {CloakBoundary} from "@mmuscat/angular-error-boundary";
import {Observable, OperatorFunction, PartialObserver, Subject, Subscription} from "rxjs";
...
export class ResourceSubject<T = any> extends Subject<ResourceSubject<T>> {
    ...
    constructor(key: string, params: Observable<any>, operator: OperatorFunction<any, any>, boundary: CloakBoundary) {
        super()
        this.key = key;
...
export function Resource([key, params]: [string, Observable<any>], factory: (key: string) => OperatorFunction<any, any>, options?: { cloak?: boolean}) {
    return new ResourceSubject(key, params, factory(key), options?.cloak ? Inject(CloakBoundary, new NoopBoundary) : new NoopBoundary);
}
//
