...
class TestSubject {
   ...
   ngOnInit() {
      if (this.throwError) {
         ...
         this.result = this.boundary
            .cloak(
               timer(this.delay, this.delay ? asyncScheduler : queueScheduler),
            )
            .pipe(mapTo(10))
      }
   }
...
function defineBoundary(Host: Type<any>) {
   TestBed.configureTestingModule({
      ...
      declarations: [Host, TestSubject, CustomFallback],
   })
...
describe("NgCloak", () => {
   ...
   it("should cloak for 500 ms (no trailing time)", fakeAsync(() => {
      TestBed.overrideProvider(NG_CLOAK_CONFIG, {
         useValue: { leading: 0, trailing: 0 },
      })
      const createBoundary = defineBoundary(Host)
      const boundary = createBoundary()
      boundary.detectChanges()
   ...
   it("should handle errors", () => {
      ...
      const cloak = getNgCloak(boundary)
      expect(cloak.nativeElement).not.toHaveClass("ng-cloak")
      expect(errorBoundary.injector.get(ElementRef).nativeElement).toHaveClass(
         "ng-cloak",
      )
      boundary.destroy()
   })
